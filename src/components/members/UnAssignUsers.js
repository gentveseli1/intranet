...
function UnAssignUsers(props) {
    ...
    const submitForm = (e) => {
        ...
        axios({
            method: "DELETE",
            url: `/google/groups/delete_members_bulk`,
            data: {
                email_arr: users,
                groupKey_arr: groups
            },
            ...
            if (res.data.deleted) {
                const success = res.data.deleted;
                success.map(success => ( 
                    toast.success(
                        <div className="toast-holder">
                            <p>🚀 Member unassigned:<br />
                                <span>{decodeURIComponent(success.email)}</span>
                            </p>
                            <p>💭 Message: successfully unassigned from<br />
                                <span>{success.groupemail}</span>!
                            </p>
                        </div>, {
            ...
            if (res.data.error) {
                ...
                errors.map(err => ( 
                    toast.error(
                        <div className="toast-holder">
                            <p>💥 Cant unassign:<br /><span>{decodeURIComponent(err.email)}</span></p>
                            <p>👨‍👨‍👦‍👦 From group:<br /><span>{err.groupemail}</span></p>
            ...
            console.log(err)
            if (err) {
                FailAlert('Users Failed to Delete');
            }
        }) 
    }
        ...
        <>
            <Container className="mt-4">
                <form onSubmit={submitForm} method="POST" className="d-flex flex-wrap">
                    ...
                    <button type="submit" className="btn btn-danger m-3">UNASSIGN USERS</button>
                </form>
            </Container>
        </>
    );
